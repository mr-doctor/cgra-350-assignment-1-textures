...
namespace cgra {
	...
	void Mesh::compute_tangents(
		...
		for (int i = 0; i < vertices.size(); i += 3) {
			...
			glm::vec2 &uv0 = vertices[i + 0].m_uv;
			glm::vec2 &uv1 = vertices[i + 1].m_uv;
			glm::vec2 &uv2 = vertices[i + 2].m_uv;

			glm::vec3 edge_1 = v1 - v0;
			glm::vec3 edge_2 = v2 - v0;

			glm::vec2 uv_edge_1 = uv1 - uv0;
			glm::vec2 uv_edge_2 = uv2 - uv0;

			float r = 1.0f / (uv_edge_1.x * uv_edge_2.y - uv_edge_1.y * uv_edge_2.x);
			glm::vec3 tangent = (edge_1 * uv_edge_2.y   - edge_2 * uv_edge_1.y)*r;
			glm::vec3 bitangent = (edge_2 * uv_edge_1.x   - edge_1 * uv_edge_2.x)*r;
	...
	void Mesh::setData(const Matrix<double> &vertices,
		...
		if (vertices.numCols() != 3 && vertices.numCols() != 5) {
		...
		std::vector<glm::vec3> normals;
		compute_tangents(m_vertices, &tangents, &bitangents, &normals);

		for (int i = 0; i < m_vertices.size(); ++i) {
			m_vertices[i].m_tangent = tangents[i];
			m_vertices[i].m_bitangent = bitangents[i];
			m_vertices[i].m_normal = normals[i];
		}
